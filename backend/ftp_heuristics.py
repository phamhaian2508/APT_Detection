...
class FTPBruteForceHeuristic:
    def __init__(
        self,
        window_seconds: int = 60,
        min_attempts: int = 4,
        service_min_attempts: int = 1,
        max_flow_duration_us: int = 20_000_000,
        service_max_flow_duration_us: int = 15_000_000,
        max_bwd_payload_bytes: int = 260,
        max_packet_len_bytes: int = 320,
        base_probability: float = 0.62,
    ...
    def evaluate(self, record: Dict[str, object], current_prediction: str) -> FTPHeuristicMatch | None:
        ...
        required_attempts = self.service_min_attempts if self._is_server_side_ftp_service(record) else self.min_attempts
        if attempts < required_attempts:
            return None

        score = min(self.base_probability + ((attempts - required_attempts) * self.probability_step), 0.98)
        return FTPHeuristicMatch(
            classification=translate_prediction_label("FTP-Patator"),
            probability=score,
    ...
    def _candidate_key(self, record: Dict[str, object]) -> Tuple[Tuple[str, str, int], float] | None:
        ...
        if protocol != "TCP":
            return None
        if src_port != FTP_PORT and dest_port != FTP_PORT:
            return None
        ...
        control_heavy = max_packet_len <= self.max_packet_len_bytes or psh_flags <= 2
        ...
        return (client_ip, server_ip, FTP_PORT), event_time

    def _is_server_side_ftp_service(self, record: Dict[str, object]) -> bool:
        ...
        return any(marker in process_name for marker in known_process_markers)
